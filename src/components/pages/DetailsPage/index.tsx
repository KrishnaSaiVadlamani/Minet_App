...
const DetailsPage = () => {
  ...
  useEffect(() => {
    fetchWatchList()
      .then(async (data) => {
        data.includes(coinId!)
          ? setWatchListId(coinId!)
          : setWatchListId(coinId!);
      })
  ...
  return (
    <Grid container>
      <Grid item xs={12} data-testid="currencyDetails">
        <CurrencyDetailCardComponent
          coinIcon={pictures[coinData.icon]}
          coinName={coinId!}
          coinSymbol={coinData.coinSymbol}
          percentageChange={coinData.percentageChange}
          isAddedtoWishList={coinData.isWatchlist}
          marketCap={coinData.marketCap}
          volumeIn24H={coinData.volume}
          circulatingSupply={coinData.totalSupply}
      ...
      <Grid item xs={12}>
        <Tab
          ...
          currencyData={coinData.currencyData}
          percentageChange={coinData.percentageChange ?? 0}
          categories={coinData.categories}
          coinBalance={coinData.coinBalance}
          currentValue={coinData.currentValue}
          coinName={coinId!}
